...
- Derive ABI splits from android.defaultConfig.ndk.abiFilters so the committed single-ABI configuration and optional multi-ABI worktree remain valid. Keep signed universal and single-ABI outputs for every release flavor, including a single supported ABI. AGP permits NDK filters with ABI splits only when the universal output is enabled. The release helper refuses stale APKs, incomplete matrices, unknown files and unmanaged destination directories. Legacy Android Studio `app/release` and `app/releases` directories are not release inputs.
