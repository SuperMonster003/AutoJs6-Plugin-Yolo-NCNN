...
- Backend d'inférence CPU NCNN 20260526 : nombre de threads réglable (par défaut 4, jusqu'à 64), pour les appareils `arm64-v8a, armeabi-v7a, x86, x86_64`.
...
- **Installer** — Ce dépôt est public, mais aucune Release publique n'a encore été publiée et le plugin ne figure pas encore dans l'index officiel des plugins (voir la section État du projet ci-dessous) : le téléchargement public et l'entrée dans l'index arriveront une fois l'APK signé pour la production et la validation finale sur appareil terminés, et la publication officielle de l'hôte AutoJs6 6.8.0 (build 5275) n'est plus un prérequis strict. D'ici là, vous pouvez construire vous-même un candidat TEST-SIGNED comme décrit dans la section Compilation et l'associer à un APK de test AutoJs6 utilisant le même certificat de débogage ; hôte et plugin doivent être signés avec le même certificat, et le code de version de l'hôte doit être au moins 5275.
- **Activer** — L'activation, la confiance et la sélection appartiennent toujours à l'hôte AutoJs6 : lorsqu'un paquet signé avec le certificat officiel AutoJs6 est installé via le centre des plugins de l'hôte, le centre des plugins écrit l'enregistrement d'activation et exécute le sondage d'activation automatiquement après l'installation ; les candidats TEST-SIGNED, les installations via adb, les mises à jour et les autres signataires nécessitent toujours une autorisation et une activation manuelles dans le centre des plugins. L'exécution de l'hôte ne lit que l'enregistrement d'activation sauvegardé et considère son absence comme désactivé (la route YOLO est désactivée par défaut). Côté script, `yolo.load` exige aussi la chaîne de composant explicite dans `options.component` ; il n'existe aucun repli implicite.
...
- **Dépanner** — Les exceptions levées par `yolo.load` et `detector.detect` portent des catégories d'erreur stables : `COMPONENT_REQUIRED` (composant absent), `PROVIDER_UNAVAILABLE` (l'hôte ne trouve pas le provider ou ne lui fait pas confiance), `MODEL_REJECTED` (modèle ou manifeste refusé à la validation ; les détails portent des préfixes comme `MANIFEST_*`), `UNSUPPORTED_CAPABILITY` (capacité demandée hors CPU/detect), `SESSION_CLOSED`, `DETECT_FAILED`, etc. Consultez la section Limites ci-dessous et la [spécification du manifeste de modèle](https://github.com/SuperMonster003/AutoJs6-Plugin-Yolo-NCNN/blob/master/docs/model-manifest-v1.md) pour le débogage.
...
- ABI prises en charge : `arm64-v8a, armeabi-v7a, x86, x86_64` ; le fichier APK universal contient la bibliothèque native de chaque ABI.
...
- L'activation, la confiance et la sélection appartiennent toujours à l'hôte AutoJs6 : seul un paquet signé avec le certificat officiel et installé via le centre des plugins reçoit automatiquement son enregistrement d'activation, toute autre voie d'installation exige une activation manuelle, et l'exécution considère l'absence d'enregistrement comme désactivé.
