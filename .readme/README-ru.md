...
- CPU-бэкенд NCNN 20260526: настраиваемое число потоков (по умолчанию 4, до 64), для устройств `arm64-v8a, armeabi-v7a, x86, x86_64`.
...
- **Установка** — Репозиторий уже публичный, но публичный Release ещё не опубликован, а плагин ещё не внесён в официальный индекс плагинов (см. раздел Статус проекта ниже); публичная загрузка и запись в индексе появятся после готовности APK с продакшен-подписью и финальной проверки на устройстве, а официальный выпуск хоста AutoJs6 6.8.0 (сборка 5275) больше не является обязательным предварительным условием. До этого можно самостоятельно собрать кандидат TEST-SIGNED по разделу Сборка и установить его в паре с тестовым APK AutoJs6 с тем же отладочным сертификатом; хост и плагин должны быть подписаны одним сертификатом, а код версии хоста должен быть не ниже 5275.
- **Включение** — Включение, доверие и выбор всегда принадлежат хосту AutoJs6: когда пакет, подписанный официальным сертификатом AutoJs6, устанавливается через центр плагинов хоста, центр плагинов после установки автоматически записывает запись о включении и выполняет проверку включения; кандидаты TEST-SIGNED, установка через adb, обновления и другие подписи по-прежнему требуют ручной авторизации и включения в центре плагинов. Среда выполнения хоста читает только сохранённую запись о включении и считает её отсутствие выключенным состоянием (маршрут YOLO по умолчанию выключен). Со стороны скрипта `yolo.load` также требует явную строку компонента в `options.component`; неявного отката нет.
...
- Поддерживаемые ABI: `arm64-v8a, armeabi-v7a, x86, x86_64`; universal APK содержит нативную библиотеку для каждой ABI.
...
- Включение, доверие и выбор всегда принадлежат хосту AutoJs6: только пакет, подписанный официальным сертификатом и установленный через центр плагинов, получает запись о включении автоматически, любой другой путь установки требует ручного включения, а среда выполнения считает отсутствие записи выключенным состоянием.
...
Рекомендуется JDK 21+; Android SDK должен предоставлять platforms 24 и 37, а также NDK 29.0.14206865 и CMake 3.22.1 (нужны для JNI NCNN). Основные команды:
...
* `Исправление` Добавлен стандартный вход активации Wake и сведения о нативных ABI установленного APK с полными локализованными описаниями
