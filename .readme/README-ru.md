...
- Контроль тайм-аутов и жизненного цикла: и открытие модели, и каждая детекция имеют потолки тайм-аута; `detector.close()` и остановка скрипта немедленно освобождают сессию и нативные ресурсы.
...
Требуется AutoJs6 с кодом версии не ниже 5275 (то есть 6.8.0 или новее), подписанный тем же сертификатом, что и плагин; Android 24+ (Android 7.0), targetSdk 36; устройство должно быть `arm64-v8a`. Версия протокола плагина 1.0; текущая версия провайдера 0.1.0 (код версии 2).
...
* [CHANGELOG-ru.md](https://github.com/SuperMonster003/AutoJs6-Plugin-Yolo-NCNN/blob/master/.changelog/CHANGELOG-ru.md)
