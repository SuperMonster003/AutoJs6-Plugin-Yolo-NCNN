...
- **설치** — 이 저장소는 공개되었지만 아직 공개 Release가 발행되지 않았고 공식 플러그인 인덱스에도 등재되지 않았습니다 (아래 프로젝트 상태 절 참조). 공개 다운로드와 인덱스 등재는 프로덕션 서명 APK와 실기기 최종 검증이 완료된 뒤 제공되며, 호스트 AutoJs6 6.8.0 (빌드 5275)의 정식 출시는 더 이상 필수 전제 조건이 아닙니다. 그 전에는 아래 빌드 절에 따라 TEST-SIGNED 후보를 직접 빌드하여 같은 디버그 인증서를 쓰는 AutoJs6 테스트 APK와 짝지어 설치할 수 있습니다. 호스트와 플러그인은 동일 인증서로 서명되어야 하며, 호스트 버전 코드는 5275 이상이어야 합니다.
- **활성화** — 활성화, 신뢰, 선택 상태는 항상 AutoJs6 호스트가 보유합니다: AutoJs6 공식 인증서로 서명된 패키지를 호스트의 플러그인 센터에서 설치하면 설치 완료 후 플러그인 센터가 활성화 기록을 자동으로 기록하고 활성화 탐색을 실행합니다. TEST-SIGNED 후보, adb 사이드로드, 업데이트 및 기타 서명은 계속 플러그인 센터에서 수동 승인과 활성화가 필요합니다. 호스트 런타임은 저장된 활성화 기록만 읽으며 기록이 없으면 꺼진 것으로 간주합니다 (YOLO 경로는 기본 비활성). 스크립트 쪽에서도 `yolo.load`의 `options.component`에 컴포넌트 문자열을 명시해야 하며, 암묵적 폴백은 없습니다.
...
- 활성화, 신뢰, 선택 상태는 항상 AutoJs6 호스트가 보유합니다: 공식 인증서로 서명되어 플러그인 센터에서 설치된 패키지만 활성화 기록이 자동으로 기록되고, 그 밖의 설치 경로는 수동 활성화가 필요하며, 런타임은 기록이 없으면 꺼진 것으로 간주합니다.
...
AutoJs6 버전 코드 5275 이상 (즉 6.8.0 이후)이어야 하며 플러그인과 동일 인증서로 서명되어야 합니다. Android 24+ (Android 7.0), targetSdk 37. 기기는 `arm64-v8a, armeabi-v7a, x86, x86_64`여야 합니다. 플러그인 프로토콜 버전 1.0, 현재 Provider 버전 0.1.3 (버전 코드 37).
...
이 저장소는 공개되었지만 이 플러그인에는 아직 공개 Release도 공식 플러그인 인덱스 등재도 없습니다. 호환 호스트 AutoJs6 6.8.0 (5275)는 아직 정식 출시되지 않았지만 그 출시는 더 이상 이 플러그인 공개의 필수 전제 조건이 아니며, 공개 Release와 인덱스 등재는 프로덕션 서명 APK와 실기기 최종 검증을 전제로 합니다. `sign.properties`가 없으면 `assembleRelease`는 서명되지 않은 APK를 생성하며, 이는 소스/빌드 증거일 뿐 배포 가능한 산출물이 아닙니다. 현재 버전은 0.1.3 (버전 코드 37)이며, 결함은 롤백이 아니라 새 버전 코드의 전방 수정으로 해결합니다. 프로덕션 서명, 실기기 최종 검증, 공개 상태는 외부 R6 증거 아카이브가 확정합니다. 자세한 내용은 [엔지니어링 노트](https://github.com/SuperMonster003/AutoJs6-Plugin-Yolo-NCNN/blob/master/docs/engineering-notes.md)를 참조하세요.
