...
- **インストール** — 本リポジトリは公開済みですが, 公開 Release はまだ発行されておらず, 公式プラグインインデックスにも未収載です (下記のプロジェクト状況節を参照). 公開ダウンロードとインデックス登録は本番署名 APK と実機での最終検証の完了後に行われ, ホスト AutoJs6 6.8.0 (ビルド 5275) の正式リリースはもはや必須の前提条件ではありません. それまでは下記のビルド節に従って TEST-SIGNED 候補を自分でビルドし, 同じデバッグ証明書を使う AutoJs6 テスト APK とペアで導入できます. ホストとプラグインは同一証明書での署名が必須で, ホストのバージョンコードは 5275 以上が必要です.
- **有効化** — 有効化, 信頼, 選択の状態は常に AutoJs6 ホストが保持します: AutoJs6 の公式証明書で署名されたパッケージをホストのプラグインセンターから導入すると, 導入完了後にプラグインセンターが有効化記録を自動で書き込み, 有効化の探査を実行します. TEST-SIGNED 候補, adb によるサイドロード, 上書き更新, その他の署名では引き続きプラグインセンターでの手動の認可と有効化が必要です. ホストの実行時は保存済みの有効化記録のみを参照し, 記録がなければ無効として扱います (YOLO ルートは既定で無効). スクリプト側でも `yolo.load` の `options.component` にコンポーネント文字列を明示する必要があり, 暗黙のフォールバックはありません.
...
- 有効化, 信頼, 選択の状態は常に AutoJs6 ホストが保持します: 公式証明書で署名されプラグインセンターから導入されたパッケージのみ有効化記録が自動で書き込まれ, それ以外の導入経路では手動の有効化が必要で, 実行時は記録がなければ無効として扱います.
