...
- محرك استدلال NCNN 20260526 على المعالج: عدد خيوط قابل للضبط (افتراضيًا 4، وحتى 64)، لأجهزة `arm64-v8a, armeabi-v7a, x86, x86_64`.
...
- **التثبيت** — هذا المستودع عام الآن، لكن لم يُنشر بعد أي Release عام ولم تُدرج الإضافة بعد في فهرس الإضافات الرسمي (انظر قسم حالة المشروع أدناه)؛ سيتاح التنزيل العام والإدراج في الفهرس بعد اكتمال ملف APK الموقّع للإنتاج والتحقق النهائي على الجهاز، ولم يعد الإصدار الرسمي للمضيف AutoJs6 6.8.0 (البنية 5275) شرطًا مسبقًا صارمًا. حتى ذلك الحين يمكنك بناء مرشح TEST-SIGNED بنفسك كما في قسم البناء أدناه، وإقرانه بملف APK تجريبي من AutoJs6 يستخدم شهادة التصحيح نفسها؛ يجب توقيع المضيف والإضافة بالشهادة نفسها، ويجب ألا يقل رمز إصدار المضيف عن 5275.
- **التفعيل** — التفعيل والثقة والاختيار حالات يملكها مضيف AutoJs6 دائمًا: عند تثبيت حزمة موقّعة بشهادة AutoJs6 الرسمية عبر مركز الإضافات في المضيف، يكتب مركز الإضافات سجل التفعيل وينفذ فحص التفعيل تلقائيًا بعد التثبيت؛ أما مرشحات TEST-SIGNED والتثبيت عبر adb والتحديثات والموقعون الآخرون فلا تزال تتطلب تخويلًا وتفعيلًا يدويين في مركز الإضافات. لا يقرأ وقت التشغيل في المضيف سوى سجل التفعيل المحفوظ ويعامل غيابه على أنه معطّل (مسار YOLO معطّل افتراضيًا). ومن جهة السكربت، يشترط `yolo.load` أيضًا سلسلة المكوّن الصريحة في `options.component`؛ ولا يوجد أي تراجع ضمني.
...
- **استكشاف الأخطاء** — تحمل الاستثناءات الصادرة عن `yolo.load` و `detector.detect` فئات أخطاء مستقرة: `COMPONENT_REQUIRED` (لم يُحدد مكوّن)، `PROVIDER_UNAVAILABLE` (المضيف لا يجد المزوّد أو لا يثق به)، `MODEL_REJECTED` (النموذج أو الـ manifest لم يجتز التحقق؛ التفاصيل تحمل بادئات مثل `MANIFEST_*`)، `UNSUPPORTED_CAPABILITY` (طُلبت قدرة خارج CPU/detect)، `SESSION_CLOSED`، `DETECT_FAILED` وغيرها. راجع قسم الحدود أدناه و[مواصفة manifest النموذج](https://github.com/SuperMonster003/AutoJs6-Plugin-Yolo-NCNN/blob/master/docs/model-manifest-v1.md) عند التشخيص.
...
- معماريات ABI المدعومة: `arm64-v8a, armeabi-v7a, x86, x86_64`؛ تتضمن حزمة universal APK المكتبة الأصلية لكل معمارية.
...
- التفعيل والثقة والاختيار حالات يملكها مضيف AutoJs6 دائمًا: الحزمة الموقّعة بالشهادة الرسمية والمثبّتة عبر مركز الإضافات هي وحدها التي يُكتب سجل تفعيلها تلقائيًا، وأي مسار تثبيت آخر يتطلب تفعيلًا يدويًا، ويعامل وقت التشغيل غياب السجل على أنه معطّل.
...
يوصى بـ JDK 21+؛ ويجب أن يوفر Android SDK المنصتين 24 و 37، إضافة إلى NDK 29.0.14206865 و CMake 3.22.1 (لازمة لترجمة JNI الخاص بـ NCNN). الأوامر الشائعة:
